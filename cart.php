...
<html>
    ...
    <body>
                ...
                case "submit":
                    if (isset($_POST['update_click'])) { //Cập nhật số lượng sản phẩm
                        ...
                        if ($error == false && !empty($_POST['quantity'])) { //Xử lý lưu giỏ hàng vào db
                            $products = mysqli_query($connect, "SELECT * FROM `product` WHERE `id` IN (" . implode(",", array_keys($_POST['quantity'])) . ")");
                            $total = 0;
                            $orderProducts = array();
                            ...
                            $insertOrder = mysqli_query($connect, "INSERT INTO `order`(`id`, `name`, `phone`, `address`, `note`, `total`, `created_time`, `last_updated`) VALUES  (NULL, '" . $_POST['name'] . "', '" . $_POST['phone'] . "', '" . $_POST['address'] . "', '" . $_POST['note'] . "', '" . $total . "', '" . time() . "', '" . time() . "');");
                           ...
                           $insertOrder = mysqli_query($connect, "INSERT INTO `order_detail`(`id`, `order_id`, `product_id`, `quatity`, `price`, `created_time`, `last_updated`) VALUES " . $insertString . ";");
        ...
        if (!empty($_SESSION["cart"])) {
            $products = mysqli_query($connect, "SELECT * FROM `product` WHERE `id` IN (" . implode(",", array_keys($_SESSION["cart"])) . ")");
        }
