...
<section class="placed-orders">
   ...
   <div class="box-container">
      ...
      $select_orders = mysqli_query($conn, " SELECT orders.name,orders.user_id,orders.id,orders.payment_status, orders.placed_on,orders.name,orders.number,orders.email,orders.address,orders.total_products,orders.total_price,orders.method, users.name_u
      FROM orders
      INNER JOIN users ON orders.user_id = users.id ") or die('query failed');
      if(mysqli_num_rows($select_orders) > 0){
         while($fetch_orders = mysqli_fetch_assoc($select_orders)){
      ?>
      <div class="box">
         <p> User id : <span><?php echo $fetch_orders['user_id']; ?></span> </p>
         <p> User name : <span><?php echo $fetch_orders['name_u']; ?></span> </p>
         <p> Thời gian : <span><?php echo $fetch_orders['placed_on']; ?></span> </p>
         <p> Tên người đặt : <span><?php echo $fetch_orders['name']; ?></span> </p>
         <p> Số điện thoại : <span><?php echo $fetch_orders['number']; ?></span> </p>
         <p> Email : <span><?php echo $fetch_orders['email']; ?></span> </p>
         <p> Địa chỉ: <span><?php echo $fetch_orders['address']; ?></span> </p>
         <p> Số lượng : <span><?php echo $fetch_orders['total_products']; ?></span> </p>
